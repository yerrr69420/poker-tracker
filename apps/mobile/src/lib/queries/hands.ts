...
export async function fetchHandFeed(
  ...
  if (filters.tab === 'unanswered') {
    ...
    // RLS handles auth, but we also filter explicitly
  ...
  // Fetch author emails for non-anonymous posts
  const posts = data as HandPostRow[];
  const userIds = [...new Set(posts.filter((p) => !p.post_anonymous).map((p) => p.user_id))];

  let emailMap: Record<string, string> = {};
  if (userIds.length > 0) {
    // Use a simple approach: fetch from auth via admin or just display user_id prefix
    // In production you'd have a profiles table; for MVP we'll show a truncated id
    emailMap = Object.fromEntries(userIds.map((id) => [id, `user_${id.slice(0, 6)}`]));
  }
